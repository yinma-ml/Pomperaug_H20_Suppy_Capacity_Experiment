...
%***********asc+attribute, scale covariates, no other covariates:successful
DATA.Y = DATA.chosen;
DATA.Xa = [DATA.alt==1, ...
...
EstimOpt.ProjectName = '02192020wtp';
...
%: poor results, covar in utility fn are not sig
%DATA.Y = DATA.chosen;
...
%***************asc+attribute, all covariates come in as scales 
%DATA.Y = DATA.chosen;
%DATA.Xa = [DATA.alt==1,DATA.nzone1, DATA.nzone2, DATA.nzone3,DATA.towndown,DATA.townmid, DATA.townup, DATA.eco,DATA.rec,DATA.dry,...    %asc,attributes
%   -DATA.tax];  %cost attributes
% EstimOpt.NamesA = {'AllSQ', 'Zone_sensitive', 'Zone_ordinary','Zone_resilient','Shift_down','Shift_mid','Shift_up','Ecological score', 'Recreation score','Dry river miles',...
%   '- Extra tax/household/month'};
%DATA.Xs = [DATA.T1,DATA.ascend,DATA.descend,DATA.block1,DATA.incpp_50k, DATA.incpp_75k, DATA.incpp_75km,DATA.f1_flow, DATA.f3_protup, DATA.f4_CTh2O,DATA.taxgrt]; %scale covariates
%EstimOpt.NamesS = {'Treatment_1','Ascending','Descending','Block_1','Income/pp25~50k','Income/pp50~75k', 'Income/pp>75k','F1_flow','F3_protect upstream','F4_CT has enough H2O','Tax usage is guaranteed'};

%EstimOpt.ProjectName = '02182020_allscales_wtp_wtp';
%% ****************************  specifying input ****************************
...
EstimOpt.NRep = 1e7; % number of draws for numerical simulation
...
%wtp space:  end up with weired results, tested with attributes only model and allscale model. fminunc stopped because it exceeded the iteration limit.Matrix is close to singular or badly scaled. 
%Results may be inaccurate. RCOND =  7.596796e-22. 
...
%around 10 min to run this model with full variables
% 
% 
% %% ************************** MXL *******************************
% 
...
%it took 4 hours 12 minutes to run this model with full variables
